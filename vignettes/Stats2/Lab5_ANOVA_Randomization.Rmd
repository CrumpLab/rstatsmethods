...
save_F_values <- length(1000)
for(i in 1:1000){
some_data <- some_data <- tibble(subjects = 1:(levels*n_per_level),
                      IV = as.factor(rep(1:levels, each = n_per_level)),
...
The above section showed an example of simulating the null-hypothesis for a design with one independent variable and multiuple levels. The basic set-up was pretty straightforward. You randomly sample numbers from the same distribution into your different groups. Thus, the null hypothesis is that every group mean should be on average be the same, because they should all be the same as the mean of the distribution that they came from.
...
Your textbook describes the notion of the score model. Briefly, this is the idea that a single subjects score can be described in terms of a series of components that are added together. 

For example, let's say the score for subject 1 is 11. We can describe the score as the sum of the grand mean, the group mean, and the leftover error. If the grand mean was 7, the group mean was 8, then this score would be 7 (the influence of the grand mean), +1 (the influence of the group mean...to get to 8), and +4 (the leftover part) to get to 11=7+1+4.
