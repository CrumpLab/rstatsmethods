...
vignette: >
  %\VignetteIndexEntry{Lab 6 Comparisons}
...
# Multiply by the sample size per group
...
First, note that we need all of the contrasts to do this. For example, there is no way we can add just the first linear contrast and somehow get the original means back...
...
Your textbook describes several methods for "correcting" p-values for multiple comparisons to protect against the family-wise type I error rate. Some of these corrections include the Sidak, Bonferroni, Boole, and Dunn. I am going to purposefully leave this concept section mostly blank; and, discuss why in class. 
