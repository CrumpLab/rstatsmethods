...
vignette: >
  %\VignetteIndexEntry{Lab 11 Correlation}
...
-   Concepts I: Measuring correlation with a simple sum
...
Using papaja to report an r-value, `r papaja::apa_print(results)$full_result`.
...
### Sum of products and correlation
...
And then summing up the products:

```{r}
sum(X*Y)
```

The sum of products (also termed the dot product) is an *un*-standardized measure of correlation or association between variables X and Y. It is not standardized because the range of values that the sum of products can take depends on the values in X and Y variables.

Consider these questions, and assume that X contains the values from 1 to 10, and so does Y.

1.  What is the largest value that the sum of products between X and Y can take?
...
Notice in this arrangement, the smallest value of X and Y (1) are paired together, the next largest value (2) are paired, and so on up until 10. This pairing creates the largest sum of products. It is also represents a perfect positive correlation.

2.  What is the smallest value that the sum of products between X and Y can take?
...
When the numbers are arranged to produce a perfect negative correlation, the sum of the products is at it's minimum possible value.

3.  If X contains only the numbers from 1 to 10 in any order, and Y does too, what kinds of sums of products can occur?
...
```{r}
rand_1000 <- replicate(10000,cor(rnorm(10,0,1),rnorm(10,0,1)))
hist(rand_1000)
mean(rand_1000)
max(rand_1000)
min(rand_1000)
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/elFmc01cpmA" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen>
