---
title: "Lab 4 Distributions II"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Lab 4 Distributions II}
...
1. Use R to roll a fair six-sided die, and show that each number comes up equally frequently in the long run. Roll the dice 10,000 times, report how many times each number (1-6) was rolled.
...
Next, imagine that you have no idea what kind of coins were being flipped, and all you have is the sequence of flips. You start at the first coin clip, and go through all of them, each time you will use the data to update your belief about the coin.
...
3. Flip a fair coin a large number of times (say 10000). Count how many flips it took from the beginning until you got five heads in a row. For example: HTHTHHHTTTHHHHH, in this sequence it took 10 flips before HHHHH occurred (1 point).

4. On average, if you start flipping a coining, how many flips will it take before get five in a row. To solve this problem you will need to repeat the above problem many times, each time getting a new count. Conduct this simulation 10000 times, and report the mean number of flips it takes to get five heads in a row (2 points)
