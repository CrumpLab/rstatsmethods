...
vignette: >
  %\VignetteIndexEntry{Lab 12 Regression}
...
2. Concept I: Visualizing best fit
...
If you happened to have several sets of X and Y data specified in a data frame, it's not too much extra effort to plot multiple scatterplots, each with their own best fit line.
...
The above methods for plotting a regression line on a scatterplot do not show the regression equation or $R^2$ values. These could be added using [annotations](https://ggplot2-book.org/annotations.html). Or, you could try to find an extension package that might do the thing you are looking for. For example, there are numerous extensions to ggplot2, <https://exts.ggplot2.tidyverse.org/gallery/>. One of them is [ggpmisc](https://docs.r4photobiology.info/ggpmisc/), which apparently can help automate adding these kinds of details to a plot.

For example, the `stat_poly_eq` function can add the $R^2$ value.
...
## Concept I: Visualizing best fit
...
## Concept II: F, R^2, and accounting for error

@wrightCorrelationCausation is credited [on wikipedia](https://en.wikipedia.org/wiki/Coefficient_of_determination) with coining the term coefficient of determination that is used for $R^2$. It was interesting to read that paper and think about the term "coefficient of determination". Wright discussed how plain old correlations could obviously be caused by multiple causal variables, which is one reason why correlation does not equal causation. He was looking for a method to breakdown a correlation into terms of component causes or paths, and this seems to inspire the rather loaded inspirational term "coefficient of determination". I say this term is inspirational because squaring an $r$-value and using deterministic language doesn't magically allow one to identify causal forces from a correlation. Instead, the general underlying concept or problem is the task of "accounting" for variation in one measure with other variables. I put "accounting" in quotes to raise a meta-issue for consideration throughout this course...e.g., what does it mean to 'account for', or 'to explain' variance? Under what conditions can we say that one variable explained, caused, or determined variation in another?

In chapter 3, @abdiExperimentalDesignAnalysis2009 introduces F values in terms of a squared $r$ coefficient. This is a natural progression because the previous chapter was on correlation.

$F = \frac{r^2}{(1-r^2)} \times (S-2)$

At the very least this sets up an expectation that $F$ values are related to $r$ and $R^2$ values. However, in my opinion, this formula hides a conceptual linkage about 'explained' variance. Both $F$ and $R^2$ are sample statistics that provide a measure of "explained" variance.

### Explaining variance

Imagine you are trying to explain some complicated thing, and you write this formula to help you keep track of your explanation:

$\text{Total stuff to explain} = \text{Explained stuff} + \text{Unexplained stuff}$

We could also write in terms of proportions:

$1 = \frac{\text{Explained} + \text{Unexplained}}{\text{Total stuff to explain}}$

Now imagine you have some explanation, and you wanted a number that quantified the amount of explanation that you have...Assuming you could put numbers into the formula to represent any two of the terms you could have (where $\text{Total}$ is the total amount of stuff to explain).

$\text{Proportion explained} = (1- \frac{\text{Unexplained}}{\text{Total}}) = \frac{\text{Explained}}{\text{Total}}$

My point in being very general here is to say that $R^2$ and $F$ are both ratios that represent a quantity for explained variance. $R^2$ is very similar to the above ratio and is bounded from 0 (explained no variance), and 1 (explained all of the variance).
...
Let's finally look at the idea of explained variance. Previously we found $SS_total$ and $SS_residual$, but we haven't directly computed $SS_explained$:

$R^2 = (1- \frac{\text{Unexplained}}{\text{Total}}) = \frac{\text{Explained}}{\text{Total}}$

$R^2 = (1- \frac{\text{SS}_{residuals}}{\text{SS}_{Total}}) = \frac{\text{SS}_{Explained}}{\text{SS}_{Total}}$

We could use algebra to find $SS_Explained$:
...
The sum of the blue squares is equivalent to $SS_explained$. We could also write:
