...
The linear regression is performed below. There are two basic inputs `lm(formula = , data=)`. The formula supplied below is `Y_dv ~ X_iv`. The dependent variable is always placed on the left side (`Y_dv`), and the independent variable is placed on the right side (`X_dv`). Order matters. In this case, we are explaining/predicting variation in `Y_dv` as a function of variation in `X_dv`.
...
Best fit lines can easily be displayed on scatterplots with ggplot2, here is a basic example. Note that I have only supplied the dataframe with the X and Y variables. `geom_point()` adds the dots, and `geom_smooth()` computes the regression and adds the line using the `lm()` method we described above. It is possible to add confidence intervals by setting `se=TRUE`, but if it's not clear to you how the intervals are computed, you might decide not to use this default.
...
Imagine you wanted to know a proportion of explained stuff, or a ratio of how much you have explained in relation to the total amount of stuff that could be explained. The proportion would be 0 when you haven't explained anything, and go to 1 as you explain everything that could be explained.
...
The scatterplot below visualizes $SS_{total}$; but, instead of a regression line as we saw earlier, the red line represents the mean of Y. So, the deviations between each dot and the red line represent the components that are added up to produce $SS_{total}$ for Y. Specifically, the sum of the area in all the error squares is taken as the total amount of stuff that needs to be explained. Slightly more formally, the squared deviations represent departures from the mean of Y. If all of the data in Y had the same number, there would be no departures from the mean, and the mean would "explain everything", leaving nothing left for any other variable to explain.
